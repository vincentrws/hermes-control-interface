...
class TuiGatewayBridge {
  ...
  start() {
    ...
    const baseHome = process.env.HERMES_HOME || path.join(os.homedir(), '.hermes');
    const root = process.env.HERMES_PYTHON_SRC_ROOT || path.join(baseHome, 'hermes-agent');
    ...
    if (this.profile && this.profile !== 'default') {
      env.HERMES_HOME = path.join(baseHome, 'profiles', this.profile);
    } else {
      env.HERMES_HOME = baseHome;
    }
  ...
  // Lightweight message send — no session.resume, just prompt.submit
  async chatSend({ message, session_id }) {
    if (!this.ready) {
      throw new Error('TUI gateway not ready');
    }
    // Resolve internal sid from canonical session_id
    let internalSid = null;
    const canonicalSid = session_id || this._canonicalSid || null;
    if (canonicalSid) {
      for (const [sid, canon] of this._sidMap) {
        if (canon === canonicalSid) { internalSid = sid; break; }
      }
    }
    if (!internalSid) {
      // Session not yet registered in this bridge — try session.resume
      if (canonicalSid) {
        try {
          console.log(`[TUI:${this.profile}] chat.send auto-resume(${canonicalSid})`);
          const result = await this.request('session.resume', { session_id: canonicalSid });
          internalSid = result.session_id;
          const resumedId = result.resumed || canonicalSid;
          this._sidMap.set(internalSid, resumedId);
          this._canonicalSid = resumedId;
          console.log(`[TUI:${this.profile}] chat.send auto-resume OK → ${resumedId}`);
        } catch (err) {
          console.log(`[TUI:${this.profile}] chat.send auto-resume FAILED (${err.message}), creating new session`);
          const result = await this.request('session.create');
          internalSid = result.session_id;
          const newSid = result.session_key || internalSid;
          this._sidMap.set(internalSid, newSid);
          this._canonicalSid = newSid;
        }
      } else {
      ...
      if (!internalSid) {
        throw new Error('no active TUI session — send chat.start first');
      }
    }
